...
BEGIN
  ...
  l_timer_set.result_lis     := timer_stat_arr();
...
Utils.W('Timer set id = ' || p_timer_set_id);
/*
  IF g_timer_set_lis(p_timer_set_id) IS NULL THEN
    Utils.W('after test');
    l_result_lis := timer_stat_arr();
    RETURN l_result_lis;
  END IF;
*/
  IF g_timer_set_lis(p_timer_set_id).result_lis.COUNT = 0 THEN
    ...
    IF l_timer_lis IS NOT NULL THEN
      l_n_timers := l_timer_lis.COUNT;
    END IF;
    l_result_lis.EXTEND(l_n_timers + 2);
...
BEGIN
  ...
  Utils.W('after self timer' || l_t_ela || ', ' || l_timer_stat_lis.COUNT);
