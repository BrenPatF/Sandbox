...
SELECT /*+ gather_plan_statistics AJA */ LPad (node, 1 + 2* (lev - 1), '.') node, lev, path, cost, lp
  ...
  ORDER BY line_no
/
EXECUTE Utils.W(Utils.Get_XPlan (p_sql_marker => 'AJA'));
...
SELECT /*+ gather_plan_statistics ALL_INT_D */ LPad (node, 1 + 2* (lev - 1), '.') node, lev, path, cost, rnk_t, rnk, lp
  FROM paths_ranked
  ORDER BY line_no
/
EXECUTE Utils.W(Utils.Get_XPlan (p_sql_marker => 'ALL_INT_D'));
...
SELECT /*+ gather_plan_statistics ALL_INT_B */ LPad (node, 1 + 2* (lev - 1), '.') node, lev, path, cost, rnk_t, rnk, lp
  FROM paths_ranked
  ORDER BY line_no
/
EXECUTE Utils.W(Utils.Get_XPlan (p_sql_marker => 'ALL_INT_B'));
...
SELECT /*+ gather_plan_statistics ALL_SOL */ LPad (node, 1 + 2* (lev - 1), '.') node, lev, path, cost, rnk_t, rnk, lp
  FROM paths_ranked
  ORDER BY line_no
/
EXECUTE Utils.W(Utils.Get_XPlan (p_sql_marker => 'ALL_SOL'));
EXEC :SRC := 'J'
...
SELECT /*+ gather_plan_statistics J_OTHER */ LPad (node, 1 + 2* (lev - 1), '.') node, lev, path, cost, lp
  FROM paths_ranked
  WHERE rnk_t = 1
  ORDER BY line_no
/
EXECUTE Utils.W(Utils.Get_XPlan (p_sql_marker => 'J_OTHER'));
SET TIMING OFF
